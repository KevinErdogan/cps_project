	...
	public void paintComponent(Graphics g) {
		...
		for(int i =0; i < env.getWidth(); i++) {
			for(int j =0; j < env.getHeight(); j++) {
				...
				g.fillRect(MyComponent.SQUARE_SIZE * i, MyComponent.SQUARE_SIZE * (hgt_1-j), MyComponent.SQUARE_SIZE, MyComponent.SQUARE_SIZE);
		...
		g.setColor(Color.RED);
		g.fillOval(MyComponent.SQUARE_SIZE * player.getWdt(), MyComponent.SQUARE_SIZE * (hgt_1 - player.getHgt()),
				MyComponent.SQUARE_SIZE , MyComponent.SQUARE_SIZE);
